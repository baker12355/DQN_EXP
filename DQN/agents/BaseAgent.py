...
class BaseAgent(object):
    ...
    def save_w(self):
        torch.save(self.model.state_dict(), self.log_dir+'/saved_agents/model.dump')
        torch.save(self.optimizer.state_dict(), self.log_dir+'/saved_agents/optim.dump')
    
    def load_w(self):
        fname_model = self.log_dir+"/saved_agents/model.dump"
        fname_optim = self.log_dir+"/saved_agents/optim.dump"
    ...
    def save_reward(self, env_id, step, reward):
        with open(os.path.join('%s/%s.monitor.csv'%(self.log_dir, env_id)), 'a') as f:
            writer = csv.writer(f)
